...
class Model(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        # 初始化为权重为N(0,1)的高斯分布，偏置为(0,1)的随机数，并且开启梯度跟踪
        super(Model,self).__init__()
        # 用nn.Parameter(data=None, requires_grad=True)来定义模型参数
...
def bce_loss(pred, label):
    ...
    #print(los.requires_grad)
    return los
    # 注意换为平均数，这样才会和nn.BCELoss()的返回值对应，因为优化器只做参数更新，默认不除以batch_size
...
def train_val(train_file_path='data/character_classification/train_feat.npy',
        ...
        if (epoch + 1) % valInterval == 0:
            ...
            with torch.no_grad():
                for feats, labels in valloader:
                    feats, labels = feats.to(device), labels.to(device)
                    out = model(feats)
                    predictions = torch.round(out)  # 四舍五入到0或1
                    n_correct += torch.sum((predictions == labels).float())   # 找出和标签匹配的预测值的数量
                    n_ims += feats.size(0)  # 增加一个批次的大小
            print('Epoch {:02d}: validation accurancy = {:.1f}%'.format(epoch + 1, 100*n_correct/n_ims))
            
            # TODO: save model parameters in model_save_path，在某些epoch时保存模型参数
    ...
    # draw the loss curve
    plot_loss(losses)   # 绘制所有epoch的误差构成的曲线
...
def visual(model_path, file_path):
    ...
    ax2.legend(['predicted characters', 'predicted backgrounds', 'decision boundary'])
...
if __name__ == '__main__':
    ...
    # -- test the saved model
        ...
        test(model_path='saved_models/model_epoch20.pth',   # 只考察最后一次的模型参数
